...
| `ANALYSIS_WINDOW_SECONDS` | Optional seconds of video per Gemini request (300–3600) | `600`: ten-minute sections, each with caption analysis and verification |
| `ANALYSIS_CONCURRENCY` | Optional number of long-video sections analyzed at once (1–4) | `2` |
| `ANALYSIS_WORKER_AUTOSTART` | Set to `0` to stop the website starting the analysis worker | The worker starts with the website when `GEMINI_API_KEY` is set |
...
`SUPADATA_API_KEY` is required alongside the YouTube and Gemini keys. If the caption service is not
configured, analysis shows a setup message before queuing a job. If captions cannot be fetched,
analysis stops with a retryable user action instead of generating a new AI transcript. The application
never requests AI transcription. Captions may be creator-provided or auto-generated by YouTube.
Expand the transcript to search and seek cues, or inspect just the selected clip's text.

Gemini analyzes both the timestamped transcript and video, then makes a separate verification request
to check candidate moments against that same source. Results include content type, timestamped topics,
clip potential, summaries and supporting evidence. Gemini is the configured video-capable provider;
`GEMINI_VIDEO_MODEL` selects the model, and `GEMINI_FALLBACK_MODELS` supplies quota fallbacks.

**Run it:** add `SUPADATA_API_KEY`, `GEMINI_API_KEY` and `YOUTUBE_API_KEY` to `.env.local`, then run `npm run dev`. The
...
10-second overlap. Each section uses two model requests: discovery using the fetched captions, then verification. High thinking and medium media resolution favor accuracy
at the cost of latency and quota. `ANALYSIS_WINDOW_SECONDS` accepts 300-3600; shorter sections reduce
analysis context size. One section runs first to establish labels; remaining sections run with
bounded concurrency. Progress names the current phase and counts only fully verified sections.
Transient failures retry with backoff, daily quota exhaustion tries configured fallback models, and
failed sections can be retried twice from the UI. Stored transcripts are reused on retries and restart.
Opening a new video consumes API quota automatically; cancellation stops active requests.

**Accuracy safeguards.** Transcript cues must have finite, ordered, valid source timestamps. Malformed
caption responses fail rather than being presented as complete. Highlights must fit their source section
and video; speech evidence must overlap an actual transcript cue. Transcript-based results with
excerpt-relative timestamps are rejected rather than shifted speculatively. A second model pass
checks timing, qualifications, misleading summaries and unsupported suggestions. These checks do not
guarantee perfect words or factual accuracy; review source playback before using a clip. Caption text
and all video content are untrusted data, never model instructions. Highlight descriptions are paraphrases;
transcript text is labeled separately. AI clip potential is not a confidence score or replay measurement.
