...
| `SUPADATA_API_KEY` | Optional hosted caption provider | Local captions continue without Supadata |
| `TRANSCRIPT_SELF_HOSTED` | Set to `0` to disable local caption extraction | Enabled; alternates with Supadata when its key is configured |
| `TRANSCRIPT_PYTHON` | Optional Python executable path | Uses `.data/transcript-venv` if installed, otherwise Python from PATH |
| `TRANSCRIPT_PROXY_URL` | Optional HTTP(S) proxy gateway with credentials | Direct YouTube requests; may be blocked by YouTube |
| `TRANSCRIPT_LANGUAGE` | Optional preferred caption language | Available native language; never requests translation or new transcription |
...
Nothing is analyzed until someone chooses **Analyze video**; opening a video only looks up an existing
analysis. Each video is analyzed once and shared with every visitor who opens it while the result is
kept (24 hours), so repeat views spend no caption credits or model quota. The timeline
...
The worker alternates uncached caption requests between our local Python extractor and
[Supadata's native transcript API](https://docs.supadata.ai/get-transcript), starting locally. If either
fails, the other is tried in the same job. Supadata is optional; the YouTube key and at least one
analysis model key are still required. Captions may be creator-provided or auto-generated by YouTube;
the application never requests new AI transcription. Expand the transcript to search and seek cues,
or inspect just the selected clip's text.

### Local caption worker

Requires Python 3.9+ (use a currently supported Python release for new deployments). Install once:

```powershell
python -m venv .data/transcript-venv
.data/transcript-venv/Scripts/python.exe -m pip install -r scripts/transcript-requirements.txt
.data/transcript-venv/Scripts/python.exe scripts/fetch-transcript.py --check
```

On Linux/macOS, use `python3` for creation and `.data/transcript-venv/bin/python` for subsequent commands.
The Node worker automatically detects this environment. Keep `scripts/fetch-transcript.py`, its Python
environment and the application together on the worker host; the website need not run Python itself.

Check retrieval with `npm run captions:check -- VIDEO_ID DURATION_SECONDS`. This uses the configured
providers and may consume one Supadata caption credit; it does not call analysis models. It prints
only provider, language, cue count and errors, using a separate `.data/caption-check` cache/state directory.
Append `--local` to test only the local extractor (separate `.data/caption-check-local` directory).
Append `--fresh --repeat=3` to issue three uncached retrievals (maximum 10). Each fresh run uses an
isolated `.data/caption-audit-*` directory with an `audit.json` report. These are deliberate live
requests, not cache tests; omit Supadata spending by also passing `--local`. Success counts describe
the tested sample and time only, never a guarantee of future availability.
Run extractor unit checks with `.data/transcript-venv/Scripts/python.exe -B -m unittest discover -s tests -p transcript_worker_test.py`.

Keep `SUPADATA_API_KEY` in `.env.local` to use both providers. For a proxy gateway, add
`TRANSCRIPT_PROXY_URL=http://username:password@gateway.example:8000` there, URL-encoding special
characters in credentials. A rotating residential proxy service supplies and rotates exit addresses;
our worker reconnects on bounded retries. Proxy access is an external service with its own costs.
No proxy account is created automatically. Requests only fetch captions, with no account cookies or
media downloads. [Extractor proxy documentation](https://github.com/jdepoix/youtube-transcript-api#working-around-ip-bans-requestblocked-or-ipblocked-exception).

- Uncached requests alternate their first provider. Failures fall back immediately after any bounded retries.
- HTTP 402 pauses Supadata for six hours. HTTP 401/403 pauses it for one hour; HTTP 429 honors
  `Retry-After` (one minute if absent, capped at 24 hours). Other transient provider failures cool down
  for a minute. Changing the relevant API key or proxy configuration resets that provider's cooldown.
- Local blocked requests retry up to three times when a proxy is configured and twice on a direct
  connection. Transient failures, including subprocess timeouts, retry with increasing delays.
  Missing or restricted captions are not repeatedly retried.
- Provider order and cooldowns persist in `<ANALYSIS_DATA_DIR>/captions/providers.json`. The existing
  single-worker lock protects this state. Do not point multiple independent workers at that directory.
- Validated captions are cached by video, duration and language preference for 24 hours; expired entries
  and entries beyond the newest 500 are pruned on subsequent cache misses. Duplicate concurrent requests
  reuse the cache. Existing analysis jobs also retain their caption sections for restart/retry.
- Extraction is serialized, new attempts are spaced by at least one second, each Python process has a
  45-second limit, each provider has a 90-second budget, and an entire retrieval has a 210-second limit.
  This reserves time for fallback when one provider stalls. Cancellation terminates its child
  process. Worker logs show provider/outcome without proxy credentials or transcript contents.
- Temporary blocks, network errors, rate limits and exhausted credits schedule up to six automatic
  job retries. Delays increase from one minute and honor provider cooldowns. The next attempt time
  survives restart and is displayed in the UI; the worker can process other jobs while waiting.
  Retries stop before the job's 24-hour expiry. Setup errors and unavailable captions require user
  action. Automatic caption recovery does not consume the manual section-retry budget.

YouTube can block direct and proxy connections, remove captions or restrict a video. If both providers
fail, the job shows the reasons and supports retry. Successful extraction cannot be guaranteed. Supadata
credit exhaustion alone no longer disables local extraction, but Claude/Gemini quotas still apply.

Verification on 2026-09-15: local-only extraction and Supadata each returned 61 timestamped English
cues for `dQw4w9WgXcQ`; an integrated check also fell back from a local failure to Supadata successfully.
Other direct attempts were blocked, so this is evidence of working retrieval, not guaranteed availability.
No paid proxy was configured or tested. Quota exhaustion, persistent cooldowns, alternation, cache
reuse, malformed captions and cancellation are covered with simulated provider responses; the Python
boundary has separate unit tests. This verification did not submit a new paid AI analysis job.

Recovery update: four fresh local requests across two videos completed with zero observed failures:
three for `dQw4w9WgXcQ` (61 cues each) and one for the app's existing job video `MgY01n03QLU` (57 cues).
All four ran outside the sandbox, without cached transcripts, Supadata or proxy credentials. These
checks do not cover every video, future YouTube changes or cloud hosting networks. Added tests cover
automatic recovery, persistence, cooldown timing, retry limits and permanent-error handling.
An additional fresh check of the app's failed video `KcCDYojg37U` returned `unavailable` (captions
missing or video restricted). The full observed sample was therefore four successes and one failure,
not a zero-failure system. Connection retry logic cannot guarantee access to unavailable captions.
...
**Run it:** install the local caption worker above, add `YOUTUBE_API_KEY` and `GEMINI_API_KEY` and/or
`ANTHROPIC_API_KEY` to `.env.local`, and keep `SUPADATA_API_KEY` for alternating hosted captions. Run `npm run dev`. The
...
Stored transcripts and the plan are reused on retries and restart. Only choosing **Analyze video**
consumes caption credits and model quota; cancellation stops active requests.
...
**Storage and sharing.** Jobs live under `.data/analysis` (or `ANALYSIS_DATA_DIR`), are kept for 24
hours, and resume after a worker restart. Analyses cover public videos and hold no visitor data, so any
visitor can look one up by video and read it; the starter's HttpOnly cookie is stored only as a hash and
never returned. Only the visitor who started an analysis can cancel it; anyone can retry its failed
sections, within the two-round limit. Starting new analyses stays limited per visitor (6 per day, one
running at a time) and overall (20 per day). Jobs from earlier analysis formats are ignored and cleaned
up. Nothing downloads the YouTube stream.
